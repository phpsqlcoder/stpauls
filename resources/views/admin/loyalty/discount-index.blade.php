    ...
    <div class="container pd-x-0">
        <div class="d-sm-flex align-items-center justify-content-between mg-b-20 mg-lg-b-25 mg-xl-b-30">
            <div>
                ...
                <h4 class="mg-b-0 tx-spacing--1">Manage Discounts</h4>
            </div>
        </div>
        ...
        <div class="row row-sm">
                ...
                <div class="filter-buttons">
                    <div class="d-md-flex bd-highlight">
                        <div class="bd-highlight mg-r-10 mg-t-10">
                            ...
                            @if (auth()->user()->has_access_to_route('discount.multiple.change.status') || auth()->user()->has_access_to_route('discount.multiple.delete'))
                            <div class="list-search d-inline">
                                <div class="dropdown d-inline mg-r-10">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        @if (auth()->user()->has_access_to_route('discount.multiple.change.status'))
                                        ...
                                        @if (auth()->user()->has_access_to_route('discount.multiple.delete'))
                        ...
                        <div class="mg-t-10">
                            <a class="btn btn-primary btn-sm mg-b-20" href="{{route('discounts.create')}}">Create a Discount</a>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <table class="table mg-b-0 table-light table-hover" style="word-break: break-all;">
                            ...
                            <tbody>
                                ...
                                <tr id="row{{$discount->id}}">
                                    <th>
                                        <div class="custom-control custom-checkbox">
                                            <input type="checkbox" class="custom-control-input cb" id="cb{{ $discount->id }}">
                                            <label class="custom-control-label" for="cb{{ $discount->id }}"></label>
                                        </div>
                                    </th>
                                    <td>
                                    ...
                                    <td>
                                        @if($discount->trashed())
                                            @if (auth()->user()->has_access_to_route('discount.restore'))
                                            <nav class="nav table-options">
                                                <a class="nav-link" href="{{route('discount.restore',$discount->id)}}" title="Restore this discount"><i data-feather="rotate-ccw"></i></a>
                                            </nav>
                                            @endif
                                        @else
                                            <nav class="nav table-options">
                                                @if (auth()->user()->has_access_to_route('discount.edit'))
                                                <a class="nav-link" href="{{ route('discounts.edit',$discount->id) }}" title="Edit Discount"><i data-feather="edit"></i></a>
                                                @endif

                                                @if (auth()->user()->has_access_to_route('discount.single.delete'))
                                                <a class="nav-link" href="javascript:void(0)" onclick="delete_one_discount('{{$discount->id}}')" title="Delete Discount"><i data-feather="trash"></i></a>
                                                @endif

                                                @if (auth()->user()->has_access_to_route('discount.change-status'))
                                ...
                                @empty
                                    <tr><th colspan="8"><center>No discounts found.</center></th></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
            ...
            <div class="col-md-6">
                <div class="mg-t-5">
                    @if ($discounts->firstItem() == null)
                        <p class="tx-gray-400 tx-12 d-inline">{{__('common.showing_zero_items')}}</p>
                    @else
                        <p class="tx-gray-400 tx-12 d-inline">Showing {{ $discounts->firstItem() }} to {{ $discounts->lastItem() }} of {{ $discounts->total() }} items</p>
                    @endif
                </div>
            </div>
            <div class="col-md-6">
                <div class="text-md-right float-md-right mg-t-5">
                    <div>
                        {{ $discounts->appends((array) $filter)->links() }}
                    </div>
                </div>
            </div>
        </div>
...
@section('pagejs')
    ...
    <script>
        let listingUrl = "{{ route('discounts.index') }}";
...
@section('customjs')
    <script>
        ...
        function post_form(url,status,discountId){
            $('#posting_form').attr('action',url);
            $('#discounts').val(discountId);
            $('#status').val(status);
            $('#posting_form').submit();
        }

        function delete_one_discount(id){
            $('#prompt-delete').modal('show');
            $('#btnDelete').on('click', function() {
                post_form("{{route('discount.single.delete')}}",'',id);
            });
        }
        ...
        function change_status(status){
            ...
            $(".cb:checked").each(function(){
                counter++;
                fid = $(this).attr('id');
                selected_discounts += fid.substring(2, fid.length)+'|';
            });
            if(parseInt(counter) < 1){
                ...
                if(parseInt(counter)>1){ // ask for confirmation when multiple pages was selected
                    ...
                    $('#btnUpdateStatus').on('click', function() {
                        post_form("{{route('discount.multiple.change.status')}}",status,selected_discounts);
                    });
                }
                else{
                    post_form("{{route('discount.multiple.change.status')}}",status,selected_discounts);
        ...
        function delete_discounts(){
            ...
            if(parseInt(counter) < 1){
                ...
                $('#btnDeleteMultiple').on('click', function() {
                    post_form("{{route('discount.multiple.delete')}}",'',selected_discounts);
                });
            }
        }
