            ...
            <div>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-style1 mg-b-10">
                        ...
                        <li class="breadcrumb-item active" aria-current="page"><a href="{{route('email-templates.index')}}">Email Templates</a></li>
                        <li class="breadcrumb-item active" aria-current="page">Create Template</li>
                    </ol>
                </nav>
                <h4 class="mg-b-0 tx-spacing--1">Create a Template</h4>
            </div>
        </div>
        <form autocomplete="off" id="albumForm" method="POST" action="{{ route('email-templates.store') }}" enctype="multipart/form-data">
            <div class="row row-sm">
                ...
                <div class="col-lg-6">
                    ...
                    <div class="form-group">
                        ...
                        <select required name="transaction_id" id="transaction_id" class="selectpicker mg-b-5 @error('transaction_id') is-invalid @enderror" value="{{ old('transaction_id') }}" data-style="btn btn-outline-light btn-md btn-block tx-left" title="Select transaction" data-width="100%">
                            @foreach($transactions as $transaction)
                                <option value="{{$transaction->id}}">{{strtoupper($transaction->name)}}</option>
                            @endforeach
                        </select>
                        @hasError(['inputName' => 'transaction_id'])
                        @endhasError
                    </div>
                </div>
                ...
                <div class="col-lg-12 mg-t-30">
                    <button type="submit" class="btn btn-primary btn-sm btn-uppercase">Save Template</button>
                    <a href="{{ route('email-templates.index') }}" class="btn btn-outline-secondary btn-sm btn-uppercase">Cancel</a>
                </div>
